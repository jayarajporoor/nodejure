...
function _execute_queries(easyDb) {
        ...
        function (err) {	
            if (err) {
                sh.error("Query failed: " + JSON.stringify(queryParam) + " error: " + err
                						 + " Query String: " + queryCtx.queryStr);
	...
	if(easyDb.proxy[fnName]){
		...
		query = easyDb.proxy[fnName].apply(easyDb.proxy, queryParam);
	}else{
		query = easyDb.proxy.genericQuery(fnName, queryParam, callback);
	}   

	queryCtx.queryStr = (query && query.sql);//TODO: right now only node-mysql support here.
	//console.log(queryCtx.queryStr);
}
...
module.exports = function (name, parentDomain, mod, extraMods) {
	...
	if(extraMods)
	{
		...
		{
			for(var i=0;i<extraMods.length;i++)
			{
				annotations.push(extraMods[i].annotations);
			}
		}else
		{
			annotations.push(extraMods.annotations);
		}
	}
	var maxDepth = 4;
